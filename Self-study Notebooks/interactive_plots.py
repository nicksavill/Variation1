    ...
    def __init__(self):
        ...
        def update_slider(slider_value):
            ...
            for a in h[pars_key(slider_value.new)]:
                a.set_visible(True)
            ax.set_title(
                f'Masses of {slider_value.new:.0f} Alaskan sockeye salmon')
...
beak sizes""",
        ...
        def pop_dist(axs, mu, sigma):
            """Population distribution"""
            ax = axs['h']
            x = np.linspace(mu-4*sigma, mu+4*sigma, 100)
            y = norm.pdf(x, mu, sigma)
            ax.fill_between(x, 0, y, color='C0', alpha=0.75)
            ax.plot([mu, mu], [0, norm.pdf(mu, mu, sigma)], color='w')
            ax.axis('on')
            ax.spines['top'].set_visible(False)
            ax.spines['right'].set_visible(False)
            ...
            ax.set_xticklabels(['$\mu$ unknown'], fontsize=14)
            ax.set_yticks([])
            ax.set_ylim(0, 0.6)
            y = norm.pdf(mu-sigma, mu, sigma)
            ax.text(mu, y-0.02, '$\sigma$ unknown', color='w', va='top', ha='center', fontsize=9)
            ax.annotate(text='', xy=(mu-sigma, y), xytext=(mu+sigma, y), arrowprops=dict(arrowstyle='<|-|>', mutation_scale=10, color='w'))

            ax = axs['g']
            ax.annotate(text=
"""The population of beak sizes has an
unknown distribution with unknown
mean $\mu$ and unknown standard
deviation $\sigma$. Inferential statistics
aims to estimate these parameters
using a single random sample.""",
                    va='center', ha='center', fontsize=14, color='w',
                    bbox=dict(boxstyle="round", fc='C0', ec='none'),
                    xytext=(0.5, -0.1), xy=(0.5, 0.8), xycoords='axes fraction', 
                    arrowprops=dict(arrowstyle="wedge,tail_width=1.",
...
$s$, are calculated""",
        ...
        def estimate(axs, xbar, s, mu, sigma):
            ax = axs['i']
            ax.annotate(text=
"""$\\bar{x}$, $s$ and $n$ are used to find
estimates of the population 
mean $\\widehat{\mu}$, and population 
standard deviation $\\widehat{\sigma}$. These
will not exactly equal the 
true population mean $\mu$ and 
standard deviation $\sigma$.""",
                    va='center', ha='center', fontsize=14, color='w',
                    bbox=dict(boxstyle="round", fc='C2', ec='none'),
                    xy=(-0.6, 0.3), xytext=(0.7, 0.5), xycoords='axes fraction', 
                    arrowprops=dict(arrowstyle="wedge,tail_width=1.",
                    relpos=(0.0, 0.2),
                    fc='C2', ec='none', shrinkA=0)
            )
            ...
            x = np.linspace(mu-4*sigma, mu+4*sigma, 100)
            y = norm.pdf(x, mu, sigma)
            ax.fill_between(x, 0, y, color='C0', alpha=0.25)
            y = norm.pdf(x, xbar, s)
            ax.fill_between(x, 0, y, color='C2', alpha=0.75)
            # ax.plot(x, y, color='C2')
            ax.tick_params(axis='x')
            ax.set_xticks([xbar])
            ax.set_xticklabels(['$\\widehat{\mu}$'], fontsize=14, color='C2')
            ax.plot([xbar, xbar], [0, norm.pdf(xbar, xbar, s)], color='w')
            ax.set_yticks([])
            ax.set_xlim(mu-4*sigma, mu+4*sigma)
            ax.set_ylim(0, 0.6)
            y = norm.pdf(xbar-s, xbar, s)
            ax.annotate(text='', xy=(xbar-s, y), xytext=(xbar+s, y), arrowprops=dict(arrowstyle='<|-|>', mutation_scale=10, color='w'))
            ax.text(xbar, y-0.02, '     $\\widehat{\sigma}$', color='w', va='top', ha='center', fontsize=14)
        ...
        mosaic = \
            ...
            .g...
            '''
        gs_kw = dict(width_ratios=[0.25, 1, 0.25, 1, 1], height_ratios=[0.25, 0.5, 1, 0.5, 1, 0.5])
...
class Distribution_of_sample_mean:
    def __init__(self):
        ...
        # declare slider for sample size
        s_prop = slider_props(10, 300, 10, 10)
        slider = FloatSlider(min=s_prop.minv, max=s_prop.maxv, step=s_prop.step, value=s_prop.init,
        ...
        for n in range(s_prop.minv, s_prop.maxv+s_prop.step, s_prop.step):
            ...
            a = ax.annotate(text='', xy=(mu-sem, y), xytext=(mu+sem, y), visible=False, 
